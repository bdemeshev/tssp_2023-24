...
\chead{}
\rhead{Midterm, 2023-11-27-retake}
\lfoot{}
\cfoot{}
\rfoot{}
...
\begin{enumerate}
  ...
  Each minute she randomly moves to an adjacent vertex with probabilities $\P(A \to B) = 0.6$, 
  $\P(A \to C) = 0.4$, $\P(B \to C) = \P(B \to A) = 0.5$,  $\P(C \to B) = \P(C \to A) = 0.5$.
  ...
  \begin{enumerate}
    ...
    \item Plot the probability $\P(X_t = X_{10})$ as a function of $t$.
    \item Plot the variance $\Var(X_t - X_{10})$ as a function of $t$.
  \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \item Find $\E(I \mid Y)$ and $\Var(I \mid Y)$.
    \item Find $\Cov(I, Y)$.
  \end{enumerate}
  ...
  \item The joint distribution of $X$ and $Y$ is given in the table
    ...
    \begin{enumerate}
     \item Explicitely find the $\sigma$-algebra $\sigma(\min\{X+Y, 0\})$.
